...
(function() {
  // Modal elements
  const modalOverlay = document.getElementById('chat-bg-customizer-modal-overlay');
  const modal = modalOverlay && modalOverlay.querySelector('.chat-bg-customizer-modal');
  const closeBtn = modalOverlay && modalOverlay.querySelector('.chat-bg-customizer-close');
  const enableToggle = modalOverlay && modalOverlay.querySelector('#bg-enable-toggle');
  const darkOverlayToggle = modalOverlay && modalOverlay.querySelector('#bg-dark-overlay-toggle');
  const particleToggle = modalOverlay && modalOverlay.querySelector('#bg-particle-toggle');
  const brightnessSlider = modalOverlay && modalOverlay.querySelector('#bg-brightness-slider');
  const blurSlider = modalOverlay && modalOverlay.querySelector('#bg-blur-slider');
  const opacitySlider = modalOverlay && modalOverlay.querySelector('#bg-opacity-slider');
  const fileInput = modalOverlay && modalOverlay.querySelector('#bg-file-input');
  const uploadBtn = modalOverlay && modalOverlay.querySelector('.chat-bg-customizer-upload-btn');
  const previewContainer = modalOverlay && modalOverlay.querySelector('#bg-preview-container');
  const resetBtn = modalOverlay && modalOverlay.querySelector('.chat-bg-customizer-reset');
  const applyBtn = modalOverlay && modalOverlay.querySelector('.chat-bg-customizer-apply');
  ...
  // Load settings from Supabase (and cache in localStorage)
  async function loadBgSettings() {
    ...
    if (data) {
      bgSettings = {
        ...
        darkOverlay: data.dark_overlay,
        particle: data.particle_effect,
      ...
      const saved = localStorage.getItem('chat_bg_settings');
      if (saved) {
  ...
  // Save settings to Supabase (and cache in localStorage)
  async function saveBgSettings() {
    const user_id = getCurrentUserId();
    if (!user_id) return;
    const upsertData = {
      user_id: user_id,
      background_url: bgSettings.url,
      background_type: bgSettings.type,
      brightness: bgSettings.brightness,
      blur: bgSettings.blur,
      opacity: bgSettings.opacity,
      dark_overlay: bgSettings.darkOverlay,
  ...
  function applyChatBackground() {
    ...
    // Only apply if enabled and url exists
    if (bgSettings.enabled && bgSettings.url) {
      if (bgSettings.type === 'image') {
        bgEl = document.createElement('img');
        bgEl.src = bgSettings.url;
        bgEl.className = 'chat-bg-customizer-bg';
        ...
        bgEl.style.opacity = bgSettings.opacity;
        bgEl.style.filter = `brightness(${bgSettings.brightness}) blur(${bgSettings.blur}px)`;
        chatSection.prepend(bgEl);
      ...
      if (bgSettings.darkOverlay) {
        ...
        overlayEl.className = 'chat-bg-customizer-overlay';
  ...
  // Update modal UI from state
  function updateModalUI() {
    enableToggle.checked = !!bgSettings.enabled;
    darkOverlayToggle.checked = !!bgSettings.darkOverlay;
    particleToggle.checked = !!bgSettings.particle;
    brightnessSlider.value = bgSettings.brightness;
    blurSlider.value = bgSettings.blur;
  ...
  // Attach open to chat header button
  document.addEventListener('click', function(e) {
    if (e.target.closest('.chat-header-bg-btn')) {
      openModal();
    }
  });
  // Close button
  ...
  if (modalOverlay) modalOverlay.onclick = function(e) {
    if (e.target === modalOverlay) closeModal();
  };
  // Escape key to close
  document.addEventListener('keydown', function(e) {
    if (modalOverlay && modalOverlay.classList.contains('active') && e.key === 'Escape') closeModal();
  });

  // Toggles and sliders
  if (enableToggle) enableToggle.oninput = function() {
    bgSettings.enabled = enableToggle.checked;
    updateModalUI();
  };
  if (darkOverlayToggle) darkOverlayToggle.oninput = function() {
    bgSettings.darkOverlay = darkOverlayToggle.checked;
    updateModalUI();
  };
  if (particleToggle) particleToggle.oninput = function() {
    bgSettings.particle = particleToggle.checked;
    updateModalUI();
  };
  if (brightnessSlider) brightnessSlider.oninput = function() {
    bgSettings.brightness = parseFloat(brightnessSlider.value);
    updateModalUI();
  };
  if (blurSlider) blurSlider.oninput = function() {
    bgSettings.blur = parseFloat(blurSlider.value);
    updateModalUI();
  };
  if (opacitySlider) opacitySlider.oninput = function() {
    bgSettings.opacity = parseFloat(opacitySlider.value);
    updateModalUI();
  };
